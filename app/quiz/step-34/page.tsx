...
function Step34Content() {
  const router = useRouter()
  const searchParams = useSearchParams()

  const answerOptions = [
    { id: "absolutely", emoji: "😫", text: "Absolutely" },
    { id: "hard_to_imagine", emoji: "😔", text: "It's hard to imagine anything else" },
    { id: "difficult_question", emoji: "🤔", text: "It's a difficult question" },
  ]

  // A imagem mostra 27/37, mas manteremos a consistência com 38
  const currentStep = 27
  const totalSteps = 38
  const progressPercentage = (currentStep / totalSteps) * 100

  const handleSelectOption = (optionId: string) => {
    const params = new URLSearchParams(searchParams.toString())
    params.set("best_relationship", optionId)
    // Navega para a próxima etapa, por exemplo, step-35
      ...
      <main className="flex-grow flex flex-col items-center justify-center p-6 text-center">
        <div className="w-full max-w-md">
          <h1 className="text-2xl font-bold text-gray-800 mb-8 leading-snug">
            Do you feel this relationship is the best you can have?
          </h1>
