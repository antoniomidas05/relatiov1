...
function Step36HContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const [selectedAdmirations, setSelectedAdmirations] = useState<string[]>([])

  const admirationOptions = [
    { id: "self_confidence", emoji: "😎", text: "My self-confidence" },
    { id: "humor", emoji: "😄", text: "My sense of humor" },
    { id: "intelligence", emoji: "🤓", text: "My intelligence" },
    { id: "generosity", emoji: "😊", text: "My generosity" },
    { id: "appearance", emoji: "😏", text: "My appearance" },
    { id: "passion_drive", emoji: "🔥", text: "My passion and drive" },
  ]

  const currentStep = 29
  const totalSteps = 38 // A imagem mostra /37, mas manteremos a consistência
  const progressPercentage = (currentStep / totalSteps) * 100

  const handleSelect = (optionId: string) => {
    setSelectedAdmirations((prev) =>
  ...
  const handleContinue = () => {
    ...
    params.set("ex_admirations", selectedAdmirations.join(","))
    // Navega para a próxima etapa, por exemplo, step-37
    router.push(`/quiz/step-37-h?${params.toString()}`)
  }
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <main className="flex-grow flex flex-col items-center p-6 text-center overflow-y-auto">
        <div className="w-full max-w-md">
          <h1 className="text-2xl font-bold text-gray-800">What did your Ex admire about you at the beginning of your relationship?</h1>
          <p className="text-gray-500 mb-6">(Choose all that apply)</p>
          <div className="space-y-3">
            {admirationOptions.map((option) => {
              const isSelected = selectedAdmirations.includes(option.id)
              return (
                <button
                  key={option.id}
      ...
      <footer className="w-full p-4 bg-gray-100 border-t border-gray-200">
        <div className="w-full max-w-md mx-auto">
          <button
            ...
            disabled={selectedAdmirations.length === 0}
...
export default function Step36H() {
  return (
