...
function Step40MContent() {
  const router = useRouter()
  const searchParams = useSearchParams()

  const handleContinue = () => {
    // Aponte para a próxima etapa final
    router.push(`/quiz/step-41-m?${searchParams.toString()}`)
  }
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <main className="flex-grow flex flex-col items-center justify-center p-6 text-center pb-28">
        <div className="w-full max-w-md flex flex-col items-center">
          ...
          <h2 className="text-3xl font-bold text-gray-800 mb-4">Make him addicted to you 🔥</h2>
          <div className="max-w-sm space-y-4">
            <p className="text-gray-700 text-lg leading-relaxed">
              We've identified the <span className="text-red-500 font-semibold">key buttons</span> that women need to
              push in order to engage with a{" "}
              <span className="text-red-500 font-semibold">man's hard-wired feelings and desires.</span> These buttons
...
export default function Step40M() {
  return (
