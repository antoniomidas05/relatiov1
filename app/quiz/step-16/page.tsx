...
function Step16Content() {
  ...
  // Lendo o gênero para personalizar as opções de resposta
  ...
  const answerOptions = [
    { id: "was_broken_up_with", text: "I was broken up with" },
    { id: "i_broke_up", text: `I broke up with ${partnerPronoun}` },
    { id: "mutual", text: "Breakup was mutual" },
    { id: "other", text: "Other" },
  ]
  ...
  const handleSelectOption = (optionId: string) => {
    ...
    params.set("breakup_initiator", optionId)
    // Navega para a próxima etapa, por exemplo, step-17
    router.push(`/quiz/step-17?${params.toString()}`)
  }
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <main className="flex-grow flex flex-col items-center justify-center p-6 text-center">
        <div className="w-full max-w-md">
          <h1 className="text-2xl font-bold text-gray-800 mb-8">Tell us more about the breakup</h1>
          <div className="space-y-3">
            {answerOptions.map((option) => (
              <button key={option.id} onClick={() => handleSelectOption(option.id)} className="w-full p-4 rounded-full text-center bg-white hover:bg-gray-50 transition-colors duration-200">
