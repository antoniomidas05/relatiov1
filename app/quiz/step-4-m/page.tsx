...
const BackArrowIcon = () => ( <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-gray-700" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}> <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /> </svg> )
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
        ...
        <div className="w-full max-w-md">
          ...
          <div className="space-y-3">
            {decisionOptions.map((option) => (
              <button key={option.id} onClick={() => setSelectedOption(option.id)} className={`w-full p-3.5 rounded-full flex justify-between items-center transition-all duration-200 ${ selectedOption === option.id ? "bg-blue-100 border border-blue-500" : "bg-white border border-transparent hover:bg-gray-50" }`}>
                <div className="flex items-center gap-4">
                  <span className="text-2xl">{option.emoji}</span>
                  <span className={`font-semibold ${ selectedOption === option.id ? "text-blue-600" : "text-gray-700" }`}> {option.text} </span>
                </div>
                {selectedOption === option.id && ( <div className="w-7 h-7 bg-blue-500 rounded-full flex items-center justify-center"> <CheckIcon /> </div> )}
              </button>
            ))}
          </div>
        </div>
      ...
      <footer className="w-full p-4 bg-gray-100">
        <div className="w-full max-w-md mx-auto">
          <button onClick={handleContinue} disabled={!selectedOption} className="w-full bg-gradient-to-r from-blue-600 to-indigo-700 text-white font-bold py-4 px-4 rounded-full shadow-lg hover:opacity-90 transition-opacity disabled:opacity-50"> Continue </button>
        </div>
      </footer>
    </div>
  )
