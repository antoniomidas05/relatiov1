...
function Step25Content() {
  ...
  const answerOptions = [
    { id: "often", emoji: "😥", text: "Quite often" },
    { id: "sometimes", emoji: "😔", text: "Sometimes" },
    { id: "never", emoji: "😌", text: "I don't" },
  ]

  // A imagem mostra 21/37, mas manteremos a consistência com 38
  const currentStep = 21
  const totalSteps = 38
  const progressPercentage = (currentStep / totalSteps) * 100

  const handleSelectOption = (optionId: string) => {
    const params = new URLSearchParams(searchParams.toString())
    params.set("loneliness_frequency", optionId)
    // Navega para a próxima etapa, por exemplo, step-26
    router.push(`/quiz/step-26?${params.toString()}`)
  }
